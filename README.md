...
- [待解决] 对于抢占式切换时，之前使用的中断调度，虽然让任务确实能被打断并交替执行，但需要对打印函数上锁，而且这还是在用户栈上进行中断处理不安全，有可能导致栈溢出，所以我后面加了tss段，打算区分内核态与用户态，但在初始化进程上下文时，发现如果给进程设置用户态的标志，会导致GPF(通用寄存器错误)果然还是哪里有问题，需要去找找对于tss的相关资料，才有可能解决。