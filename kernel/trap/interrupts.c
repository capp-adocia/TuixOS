...
*/
// 0: 除零错误 - 可恢复，设置默认结果
void isr_divide_error_handler(struct trap_frame* frame)
{
...
// 1: 调试异常 - 可恢复，用于调试器
void isr_debug_exception_handler(struct trap_frame* frame)
{
...
// 3: 断点 - 可恢复，用于调试
void isr_breakpoint_handler(struct trap_frame* frame)
{
...
// 4: 溢出 - 可恢复，清除标志
void isr_overflow_handler(struct trap_frame* frame)
{
...
// 5: 边界检查 - 可恢复，修复索引
void isr_bounds_check_handler(struct trap_frame* frame)
{
...
// 6: 无效操作码 - 严重，无法恢复
void isr_invalid_opcode_handler(struct trap_frame* frame)
{
...
// 10: 无效TSS - 系统配置错误
void isr_invalid_tss_handler(struct trap_frame* frame)
{
...
// 11: 段不存在, 重新加载段
void isr_segment_not_present_handler(struct trap_frame* frame)
{
...
// 12: 栈段错误 - 严重内存错误
void isr_stack_segment_fault_handler(struct trap_frame* frame)
{
...
// 13: 通用保护错误 - 严重内存/权限错误
void isr_general_protection_fault_handler(struct trap_frame* frame)
{
...
// 14: 页错误 - 可恢复，处理缺页
void isr_page_fault_handler(struct trap_frame* frame)
{
    uint32_t fault_addr;
    ...
    // 解析错误码
    serial_printf("  P=%d\n", (frame->err_code & 0x1) ? 1 : 0);
    serial_printf("  W/R=%d (%s operation)\n", (frame->err_code & 0x2) ? 1 : 0);
    serial_printf("  U/S=%d (%s mode)\n", (frame->err_code & 0x4) ? 1 : 0);
...
// 17: 对齐检查 - 可修复
void isr_alignment_check_handler(struct trap_frame* frame)
{
...
// 21: 控制保护异常
void isr_control_protection_handler(struct trap_frame* frame)
{
...
// 30: 安全异常
void isr_security_exception_handler(struct trap_frame* frame)
{
...
// 32: 定时器中断
void isr_timer_handler(struct trap_frame* frame)
{
    timer_handler(frame);
}

// 33: 键盘中断
void isr_keyboard_handler(struct trap_frame* frame)
{
